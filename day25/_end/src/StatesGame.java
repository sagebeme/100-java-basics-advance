...
public class StatesGame {
    ...
    public enum GuessResult { CORRECT, ALREADY_GUESSED, INVALID }

    public static GuessResult evaluateGuess(String guess, List<String> states, List<String> guessedStates) {
        if (guessedStates.contains(guess)) {
            return GuessResult.ALREADY_GUESSED;
        }
        if (states.contains(guess)) {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (guessedStates.size() < states.size()) {
                ...
                GuessResult result = evaluateGuess(guess, states, guessedStates);
                if (result == GuessResult.CORRECT) {
                    guessedStates.add(guess);
                    System.out.println("Correct! " + guess + " added.");
                } else if (result == GuessResult.ALREADY_GUESSED) {
